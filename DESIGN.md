...
- **Header**: chrome fixo sobre scrim superior, por breakpoint; some na rota `/assistir`. **Desktop (≥ lg)**: logo + nav inline (Início · Categorias · PodCast · Zap da Fé ↗ externo) à esquerda, coin chip + Conta/Entrar à direita; item ativo em laranja. **Mobile (< lg)**: linha 1 com logo centrada e busca + avatar à direita (círculo com a inicial do e-mail logado; ícone user-circle deslogado); linha 2 com as abas (micro-label, ativa em laranja + barra underline 2px laranja). Sem drawer lateral: as abas fixas já são o menu, e 314 Pass/Coins/Conta/Termos vivem no rodapé e em `/conta` (o drawer foi removido em 2026-09-25). **Busca real** (`components/SearchOverlay.tsx`): overlay full-screen, input com hairline e autofoco, debounce 300ms, `ilike` em `series.title` via browser client, resultados com thumb + categoria (regra "Em breve" do RailCard), estados de dica/loading shimmer/vazio.
...
| `components/Logo.tsx` | Renderiza a marca dourada **sem filtro `invert`** (a arte já é para tema escuro), com `width`/`height` reais para segurar o layout no carregamento. Tamanhos: header e rodapé `h-[54px]` (172 px de largura). O login não tem logo próprio (2026-09-25: batia no do header no celular) — `lg` fica disponível, sem uso |
