...
| Asset | Origem |
|---|---|
| `assets/logo-master.png` | Master da marca (1254×1254, ícone dourado sobre preto) — **fora de `public/`**, não é servido; é a fonte de todos os ícones |
| `public/logo.png`, `app/icon.png`, `app/apple-icon.png`, `app/favicon.ico` | Gerados por `python scripts/generate-icons.py`: o master é recortado na arte (~25% de margem transparente sai fora), então a caixa do logo = a marca. `logo.png` 301×288 (94 KB) para header/login; `.ico` traz 16→256 px, com realce de contraste nos tamanhos < 64 px para o "314" sobreviver ao downscale |
| `components/Logo.tsx` | Renderiza a marca dourada **sem filtro `invert`** (a arte já é para tema escuro). Tamanhos: header `h-[54px]`, login `h-[78px]` |
