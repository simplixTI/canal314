...
- **Home `/`**: home de streaming clássica (referência do dono: reelshort.com/pt) — hero do carro-chefe (`display_order=1`) em ~88dvh com arte P&B edge-to-edge, scrims da esquerda e de baixo, conteúdo à esquerda em `max-w-[560px]`: badges (chip invertido "Novo" + chip hairline de categoria), título display gigante, descrição com clamp de 2 linhas, meta (N episódios · 1–2 min), pill branco "▶ Reproduzir" + link hairline "Teste grátis". Abaixo, fileiras horizontais roláveis (`no-scrollbar`) de mini-thumbs 16:10 (PosterArt `size="thumb"`): "Novo Lançamento", "Mais Assistidos" (ícone de chama SVG), "Religioso" e "Político". Séries futuras (0 episódios) levam chip invertido "Em breve", não são links e mostram "Em breve no catálogo" na meta. O feed vertical snap-scroll foi rejeitado pelo dono e removido (2026-09).
